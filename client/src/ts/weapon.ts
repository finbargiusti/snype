...
import {handlers, socketSend} from './net';
import {getNonLocalPlayerHitboxes, gunDownOffset, gunLength, gunRightOffset, Player, players} from './player';
import {xAxis, zAxis} from './rendering';
...
export class WeaponInstance {
  ...
  shoot() {
    ...
    if (now - this.lastShotTime >= this.weapon.timeBetweenShots / rofFactor) {
      ...
      // Check if there is something inbetween the start and the end of the
      // gun's barrel. If so, the gun is likely sticking through something, and
      // shouldn't be able to shoot.
      let raytraceDirec = origin.clone();
      raytraceDirec.sub(barrelStart);
      let raytraceLen = raytraceDirec.length();
      raytraceDirec.normalize();
      let raytracer =
          new THREE.Raycaster(barrelStart, raytraceDirec, 0, raytraceLen);
      let intersections =
          raytracer.intersectObjects(gameState.currentMap.colliders);
      ...
      for (let i = 0; i < this.weapon.spec.projectilesPerShot; i++) {
        ...
        let inacc = this.wielder.isScoped ? this.weapon.spec.inaccuracy / 2 :
        ...
        let proj = new Projectile(
            this.weapon.spec.projectileOptions, origin, direction);
        ...
        socketSend('createProjectile', {
          ...
          direction:
              {x: proj.direction.x, y: proj.direction.y, z: proj.direction.z},
...
export const ASSAULT_RIFLE = new Weapon({
  ...
  projectileOptions: {speed: 175, damage: 30, color: 0xff003f}
});
...
export const SHOTGUN = new Weapon({
  ...
  projectileOptions: {speed: 125, damage: 20, color: 0x006b3c}
});
...
export const SNIPER = new Weapon({
  ...
  inaccuracy: 0.002,
  projectilesPerShot: 1,
  projectileOptions: {speed: 250, damage: 100, color: 0x048eff}
});
...
export const SMG = new Weapon({
  ...
  inaccuracy: 0.10,
  projectilesPerShot: 1,
  projectileOptions: {speed: 125, damage: 20, color: 0xfc8327}
});

export const weapons: Weapon[] = [ASSAULT_RIFLE, SHOTGUN, SNIPER, SMG];

const MAX_PROJECTILE_LIFETIME =
    1500;  // Kill it after this many milliseconds. Always.
const TIME_FRAG =
    1 / 50;  // How "long" the projectile is, relative to its speed. If it moves
             // 5 units per second, and TIME_FRAG is 1/2, then the projectile
             // is 2.5 units long.
...
export class Projectile {
  ...
  constructor(
      options: ProjectileOptions, origin: THREE.Vector3,
    ...
    let back =
        this.direction.clone().multiplyScalar(this.options.speed * TIME_FRAG);
    lineStart.sub(back);
    let curve = new THREE.LineCurve3(lineStart, new THREE.Vector3(0, 0, 0));
    let tubeGeometry = new THREE.TubeBufferGeometry(curve, 16, 0.035, 6, true);
    let projMaterial = new THREE.MeshBasicMaterial({color: this.options.color});
  ...
  update(timeDif: number) {
    ...
    let travelled = this.direction.clone().multiplyScalar(
        (this.options.speed * timeDif) / 1000);
    ...
    if (this.shouldCheckCollision) {
      ...
      let ray = new THREE.Raycaster(
          this.lastEndPoint, this.direction, 0.01, travelled.length() * 1.01);
    ...
    if (first) {
      ...
      if (player && !this.hitPlayers.has(player)) {
        ...
        socketSend('playerHit', {id: player.id, damage: this.options.damage});
      }

      socketSend('removeProjectile', {id: this.id});
    } else {
